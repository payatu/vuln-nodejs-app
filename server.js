        ...
        Wallet.findOne({where:{username: socket.user.username}}, {attributes:['BTC', 'ETH']})
            .then((crypto_balance)=>{
                ...
                request('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin%2Cethereum&vs_currencies=usd', {json: true}, (err, res, body)=>{
                    ...
                    const bitcoin_usd_value = bitcoin_quantity * body.bitcoin.usd;
                    const ethereum_usd_value = ethereum_quantity * body.ethereum.usd;
                    const total_usd_value = bitcoin_usd_value + ethereum_usd_value;
